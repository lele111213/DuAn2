...
class ZaloPay extends StatefulWidget {
  @override
  _ZaloPayState createState() => _ZaloPayState();
}
...
class _ZaloPayState extends State<ZaloPay> {
  ...
  /// Build Button Pay
  Widget _btnPay(String zpToken, User? user) => Padding(
      padding: const EdgeInsets.symmetric(vertical: 20.0, horizontal: 20.0),
      child: Visibility(
        visible: showResult,
        child: GestureDetector(
          onTap: () async {
            ...
            try {
              ...
              response = result;
              response = jsonDecode(result)['result'];
              appTransID = jsonDecode(result)['appTransID'];
              print("payOrder Result: '$result'.");
              if (response == "Payment Success") {
                var status = await checkStatus(appTransID);
                if (status != null && status.returncode == 1) {
                  response += ' +${status.amount}';
                  if (user != null) {
                    user.addCoin(status.amount ?? 0);
                    // lưu user
      ...
      Column(
        ...
        children: <Widget>[
          Container(
            ...
            child: Text("AppID: 2554, Giá trị nạp từ 1,000 đến 5,000,000"),
          ),
        ],
      ),
      // _btnQuickEdit,
